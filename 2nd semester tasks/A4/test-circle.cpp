...
BOOST_AUTO_TEST_SUITE(TestCircle, *boost::unit_test::tolerance(0.001))
  ...
  {
    Circle circle({1, 5}, 4);
    rectangle_t frameRectBeforeRotation = circle.getFrameRect();
    double angle = 90;
    double areaBeforeRotate = circle.getArea();
    circle.rotate(angle);
    rectangle_t frameRectAfterRotation = circle.getFrameRect();
    ...
    BOOST_TEST(frameRectAfterRotation.height == frameRectBeforeRotation.height);
    BOOST_TEST(frameRectAfterRotation.width == frameRectBeforeRotation.width);
    BOOST_TEST(circle.getArea() == areaBeforeRotate);
  }
